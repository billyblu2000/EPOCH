...
创世书具体是什么：
描述这本书的核心人物，核心故事，在一个什么样的世界展开，如何展开，以及如何结束。这本书的核心爽点在哪里，是给什么样的人看的，如何合理分配故事内容和情节发展等等。
创世书应该看上去像是作者本人对这本书的评述，是这本书的总纲，要关注抽象，内核。创世书不是第三者对这本书的评价。虽然是作者本人的评述，但无需用"我"第一人称来讲述，使用“这本书...”这样的叙述方式即可。

输出要求：
1. 使用纯文本，不要使用任何 Markdown 语法（不要用 #、*、- 等标记），像小说叙述一样娓娓道来。可以用分段和空行来组织结构，但不要用列表、标题符号或加粗。
2. 如果用户提供的设定信息较少或较模糊，你需要主动进行合理的想象与补全，确保创世书中至少呈现出一个核心明确、脉络清晰的完整故事框架。不要因为用户给的少就写得空洞——用你的创造力填满空白。
3. 如果用户提供的内容已经足够丰富和具体，则忠实地围绕这些内容展开，不要删减或忽略用户已给出的设定。
4. 总字数约1500字。`;
}
